...
if ( !class_exists('sqlserver_valkyrie') )
{
	class sqlserver_valkyrie Extends PDO
	{
		...
		public function __construct(instance &$instance=NULL)
		{
			try
			{
				...
				ELSE //NO INSERT INSTANCE AND CALL DEFAULT
				{
					...
					$this->OPTIONS[PDO::ATTR_DEFAULT_FETCH_MODE] = PDO::FETCH_ASSOC; // Default : parent::FETCH_ASSOC but FETCH_OBJ stack the mysql engine
					#--[end]INITIALISATION--#
				}
				#--[end]INSTANCE--#
        ...
        public function getOS() {
            ...
            switch (true) {
                case stristr(PHP_OS, 'DAR'): return $OS_OSX;break;
                case stristr(PHP_OS, 'WIN'): return $OS_WIN;break;
                case stristr(PHP_OS, 'LINUX'): return $OS_LINUX;break;
                default : return $OS_UNKNOWN;break;
            }
        }
			...
			if(is_null($this->INSTANCE) | empty($this->INSTANCE))
			{
				try
				{
					...
					if(is_null($this->PORT) | empty($this->PORT))
					    if($this->getOS() == 2)
						    $this->INSTANCE = new PDO('sqlsrv:server='.$this->HOST.';Database='.$this->DATABASE, $this->USER, $this->PASSWORD);
                        else
                        {
                            if($this->getOS() == 3)
                                $this->INSTANCE = new PDO('dblib:version=8.0;charset=UTF-8;host='.$this->HOST.';dbname='.$this->DATABASE, $this->USER, $this->PASSWORD);
                            else
                            {
                        ...
                        if($this->getOS() == 2)
						    $this->INSTANCE = new PDO('sqlsrv:server='.$this->HOST.','.$this->PORT.';Database='.$this->DATABASE, $this->USER, $this->PASSWORD, $this->OPTIONS);
                        else
                        {
                            if($this->getOS() == 3)
                                $this->INSTANCE = new PDO('dblib:version=8.0;charset=UTF-8;host='.$this->HOST.':'.$this->PORT.';dbname='.$this->DATABASE, $this->USER, $this->PASSWORD, $this->OPTIONS);
                            else
                            {
                                throw new engine_error($lang->_('TS_BAD_FILE_INSTALL'),9);
                            }
		...
		public function LS_prepare($data)
		{
			...
			try
			{
                ...
                //SECURITY ARRAY ARGUMENT MULTIDIMENSION
                IF(is_array($data['insert_statement']) && !empty($data['insert_statement']))
                {
                    foreach($data['insert_statement'] AS $key => $value)
                    {
                        //IF(is_array($value))
                            //throw new engine_error($this->lang->_('TS_BAD_ARGUMENTS'),9);
                    }
                ...
                IF(is_array($data['insert_statement']) && !empty($data['insert_statement']))
				    $this->PROMPT->execute($data['insert_statement']);
                else
                    $this->PROMPT->execute();
